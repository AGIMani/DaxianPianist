...
class Piano(composer.Entity):
    ...
    def _add_mjcf_elements(self) -> None:
        ...
        # Top-down: 1.05x piano width, pulled back so fovy stays moderate.
        half_width = float(self._size[1])
        topdown_fovy_deg = 36.0
        topdown_distance = (1.05 * half_width) / np.tan(
            np.deg2rad(topdown_fovy_deg) / 2.0
        )
        self._mjcf_root.worldbody.add(
            "camera",
            name="topdown",
            pos=[0, 0, topdown_distance],
            quat=[1, 0, 0, 1],
            fovy=topdown_fovy_deg,
        )

    def _add_back_camera(self) -> None:
        """Behind-the-piano view covering 1.05x keyboard width without a wide lens.

        Default MuJoCo fovy is 45° from ~1 m, which crops the fallboard and looks
        barrel-distorted. Pull the camera back and use a moderate vertical FOV so
        fingertip–key contact is readable.
        """
        lookat = np.array([0.0, 0.0, piano_consts.WHITE_KEY_Z_OFFSET])
        half_width = float(self._size[1])
        fovy_deg = 36.0
        # Eval videos are 640x480 (width x height in PianoSoundVideoWrapper).
        aspect = 640.0 / 480.0
        fovy = np.deg2rad(fovy_deg)
        hfov = 2.0 * np.arctan(np.tan(fovy / 2.0) * aspect)
        distance = (1.05 * half_width) / np.tan(hfov / 2.0)
        elevation = np.deg2rad(42.0)
        offset = np.array(
            [
                -np.cos(elevation) * distance,
                0.0,
                np.sin(elevation) * distance,
            ]
        )
        pos = lookat + offset
        z_axis = offset / np.linalg.norm(offset)
